...
class TrackerCreator:
    ...
    def __init__(self, credentials_path: str = 'vipalina_google_credentials.json'):
    ...
    def _authorize(self):
        ...
        try:
            ...
            formula = f'=QUERY(IMPORTRANGE("{gant_spreadsheet_id}";"{gant_tag}!A:E");"SELECT Col1, Col4, Col5 WHERE (Col1 >= 1 AND Col1 <= 100) OR Col5 = \'Онлайн\'";0)'
            creds = None
            ...
            try:
                ...
                logger.warning(f"⚠️ Не удалось авторизоваться через User OAuth: {e}")
                creds = None
    ...
    def create_tracker(
        ...
        try:
            ...
            logger.info(f"   Курс: {tracker_name}")
            ...
            new_title = f"{student_name} - {tracker_name}"
            new_tracker_id = self._copy_template(new_title, parent_folder_id)
            ...
            # 4. Открываем скопированный трекер
            tracker = self.sheets_client.open_by_key(new_tracker_id)
            
            # 5. Копируем данные курса в лист "⚠️ Конфигурация" (ДО удаления листов)
            self._copy_course_data_to_config(tracker, tracker_name, course_params)
    ...
    def _copy_template(self, new_title: str, parent_folder_id: Optional[str] = None) -> str:
        """
    ...
    def _fill_tracker_data(
        ...
        try:
            ...
            # A1 остается пустой
            main_sheet.update([[course_name]], 'C1', value_input_option='USER_ENTERED')
            ...
            # Настраиваем лист "📚 Все уроки курса" с формулой IMPORTRANGE
    ...
    def _copy_course_data_to_config(self, tracker, course_name: str, params: Dict[str, Any]):
        """
        Копирует данные курса в лист "⚠️ Конфигурация".
        Это позволяет формулам VLOOKUP работать без IMPORTRANGE.
        """
        try:
            logger.info("⚠️ Копируем данные курса в лист 'Конфигурация'...")
            
            # Ищем лист Конфигурация (с разными вариантами названия)
            config_sheet = None
            ...
            # Данные курса в строку 2:
            # A2: Название курса
            # B2: Количество уроков
            # C2: Доступ (мес)
            # D2: Куратор (мес)
            # E2: VIP (мес)
            # F2: Тег курса (название листа в таблице ГАНТ)
            gant_tag = params.get('gant_sheet', '')
            
            course_data = [[
                course_name,
                params['lesson_count'],
                params['access_days'] // 30,  # конвертируем дни в месяцы
                params['curator_support_days'] // 30,
                params['vip_support_days'] // 30,
                gant_tag  # Тег курса для листа "📚 Все уроки курса"
            formula = f'=QUERY(IMPORTRANGE("{gant_spreadsheet_id}";"{gant_tag}!A:E");"SELECT Col1, Col4, Col5 WHERE (Col1 >= 1 AND Col1 <= 100) OR Col5 = \'Онлайн\'";0)'
            
            config_sheet.update(course_data, 'A2:F2', value_input_option='USER_ENTERED')
            logger.info(f"✅ Тег курса для ГАНТ: '{gant_tag}'")
            
            logger.info(f"✅ Данные курса '{course_name}' скопированы в Конфигурацию")
    ...
    def _update_monthly_progress_formulas(self, worksheet, params: Dict[str, Any]):
        ...
        try:
            ...
            # Получаем количество месяцев поддержки куратора
            curator_months = params['curator_support_days'] // 30
            ...
            for month in range(1, curator_months + 1):
                ...
                # Используем YEAR и MONTH для определения календарного месяца от даты начала
                # Месяц 1 = тот же месяц что C4, Месяц 2 = следующий месяц и т.д.
                fact_formula = f'=COUNTIFS(\'📊 Обучение по месяцам\'!F:F;TRUE;\'📊 Обучение по месяцам\'!E:E;">="&DATE(YEAR(EDATE(C4;{month-1}));MONTH(EDATE(C4;{month-1}));1);\'📊 Обучение по месяцам\'!E:E;"<"&DATE(YEAR(EDATE(C4;{month}));MONTH(EDATE(C4;{month}));1))'
                
                # E: Процент выполнения
                progress_formula = f'=IF(C{row_num}=0;0;ROUND(D{row_num}/C{row_num}*100;0))&"%"'
                
                # F: Статус (с новыми условиями: >=9 ✅, >6 🆗, >3 ⚠️, иначе ❌)
                status_formula = f'=IF(D{row_num}>=C{row_num};"✅";IF(D{row_num}>6;"🆗";IF(D{row_num}>3;"⚠️";"❌")))'
                
                # G: Визуальный прогресс-бар (поддерживает >100%)
                visual_formula = f'=REPT("█";MIN(10;ROUND(D{row_num}/C{row_num}*10;0)))&REPT("░";MAX(0;10-ROUND(D{row_num}/C{row_num}*10;0)))&" "&TEXT(D{row_num}/C{row_num};"0%")'
            ...
            if rows_data:
                ...
                end_row = progress_header_row + curator_months
                ...
                worksheet.update(rows_data, range_name, value_input_option='USER_ENTERED')
                logger.info(f"✅ Создано {curator_months} строк помесячного прогресса ({range_name})")
                ...
                try:
                    ...
                    if curator_months >= 1:
                        ...
                        worksheet.spreadsheet.batch_update(copy_format_request)
    ...
    def _hide_config_sheet(self, tracker):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"✅ Лист '{config_sheet.title}' скрыт")
    ...
    def _setup_lessons_sheet(self, tracker, params: Dict[str, Any]):
        """
        Настраивает лист "📚 Все уроки курса" с формулой IMPORTRANGE.
        Формула импортирует данные из таблицы ГАНТ по тегу курса.
        
        Фильтрация:
        - Только столбцы A, D, E
        - Только строки где в столбце A цифра от 1 до 100
        ...
        try:
            ...
            logger.info(f"📚 Настройка листа 'Все уроки курса' для тега: '{gant_tag}'")
            
            # Ищем лист "📚 Все уроки курса"
            lessons_sheet = None
            ...
            if lessons_sheet is None:
                logger.warning("⚠️ Не найден лист '📚 Все уроки курса'")
                return
            ...
            headers = [[
                ...
                "Формат проведения"
            formula = f'=QUERY(IMPORTRANGE("{gant_spreadsheet_id}";"{gant_tag}!A:E");"SELECT Col1, Col4, Col5 WHERE (Col1 >= 1 AND Col1 <= 100) OR Col5 = \'Онлайн\'";0)'
            lessons_sheet.update(headers, 'A1:C1', value_input_option='USER_ENTERED')
            logger.info("✅ Заголовки добавлены в строку 1")
            ...
            # Фильтруем:
            # - Только столбцы A, D, E (номер, название, формат)
            # - Только строки где:
            #   1. A от 1 до 100 (уроки с номерами)
            #   ИЛИ 2. E = 'Онлайн' (онлайн-мероприятия)
            formula = f'=QUERY(IMPORTRANGE(\"{source_spreadsheet_id}\";"{gant_tag}!A:E");"SELECT Col1, Col4, Col5 WHERE (Col1 >= 1 AND Col1 <= 100) OR Col5 = \'\u041e\u043d\u043b\u0430\u0439\u043d\'";0)'
            ...
            # Форматирование: черный шрифт, размер 12
            format_request = {
                "requests": [
                    {
                        "repeatCell": {
                            "range": {
                                ...
                                "endColumnIndex": 3  # До столбца C включительно
                            },
                            "cell": {
                                "userEnteredFormat": {
                                    "textFormat": {
            ...
            logger.info("✅ Форматирование применено: черный шрифт, размер 12")
            
            logger.info(f"✅ Лист '📚 Все уроки курса' настроен")
            logger.info(f"   Тег: {gant_tag}")
            logger.info(f"   Формула в A2: {formula[:80]}...")
            
        except Exception as e:
            logger.error(f"❌ Ошибка при настройке листа '📚 Все уроки курса': {e}")
    ...
    def add_course_to_classifier(self, getcourse_tag: str, internal_name: str) -> bool:
        """
        Добавляет новый курс в лист "Условия курсов" шаблона.
        ...
        """
        try:
            logger.info(f"📚 Добавление курса в лист 'Условия курсов': {getcourse_tag} -> {internal_name}")
            
            # Открываем шаблон
            template = self.sheets_client.open_by_key(self.TEMPLATE_ID)
            
            # Ищем лист "Условия курсов"
            try:
                conditions_sheet = template.worksheet("Условия курсов")
            except:
                logger.error("❌ Не найден лист 'Условия курсов' в шаблоне")
                return False
            
            # Проверяем, не существует ли уже такой курс
            all_values = conditions_sheet.get_all_values()
            for row in all_values[1:]:  # Пропускаем заголовок
                if len(row) >= 2 and row[0] == getcourse_tag:
                    logger.info(f"ℹ️ Курс {getcourse_tag} уже есть в листе 'Условия курсов'")
                    return True
            ...
            conditions_sheet.append_row(new_row, value_input_option='RAW')
            
            logger.info(f"✅ Курс добавлен в лист 'Условия курсов': {getcourse_tag} | {internal_name}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Ошибка при добавлении курса в лист 'Условия курсов': {e}", exc_info=True)
            return False
